...
AUTO_REPLACE_BREEZE = [
    ...
    ("繳獲式", "攪禍式", [], []),
    ("doc", "大可", [], []),
    # 5/29 技術分析-5月-02/03 學習
    ("三海軍", "30均", [], []),
    # 6/18 內耗認知決策 part1/2 學習（閒聊形式）
    ("公共彈", "高空彈", [], []),
    ("辦動", "辦桌", [], []),
    # 6/22 投資組合-5月-01 學習
    ("長頭", "長投", [], []),
    ...
    ("健康週期", "建倉週期", [], []),
    ("週三白", "週三百", [], []),
    # 7/3 投資組合-6月-03 學習（已剔除 context-blind 危險規則：
    #   回答→回檔（反咬 Q&A「回答問題」）、加強→加倉（「加強」是常用詞）、三百的→300的（過特定））
    ("一周", "一週", [], []),
    ("裡長", "裡漲", [], []),
    ("均功股", "軍工股", [], []),
    ("壓住在", "押注在", [], []),
    ...
    ("默默的", "默默地", [], []),
    ("全質鼓", "權值股", [], []),  # ASR 原詞
    ("全指股", "權值股", [], []),  # 校正 subagent 曾誤猜的變體（用戶指正）
    # 7/17 技術分析-6月-04 學習
    ("世界交易", "事件交易", [], []),
    ("關鍵架", "關鍵價", [], []),
    ("周甲摔", "週假摔", [], []),
    ...
    ("泡沫調", "泡沫掉", [], []),
    # 7/24 技術分析-7月-03 學習
    ("捷徑的", "竭盡的", [], []),  # 力道竭盡（7月-02 pptx 定案術語）
    ("季限", "季線", [], []),
    ("技甲", "季假摔", [], []),
    ("根疊", "根跌", [], []),
    ("爆音", "爆陰", [], []),
    ("個記", "個季", [], []),  # 季K 語境；裸「記→季」會誤傷「記得」，只收窄形式
    ("為記", "為季", [], []),
    # 2026-08-13 技術分析-8月-01 學習（Q3 第7課：鈍化指標與市場極端情緒）
    ("頓畫", "鈍化", [], []),
    ("的遁", "的鈍", [], []),
    ("二向性", "二象性", [], []),
    ("波利二", "波粒二", [], []),
    ...
    ("離 g 線", "離均線", [], []),
    ("離g線", "離均線", [], []),
    # 2026-08-14 技術分析-8月-02 學習
    # 「星線」：跨 7 部影片 28 處，語境全為 K 線；64 部素材中「新線」零正常用法
    ("新線", "星線", [], ["新線索", "新線路", "新線上"]),
    # 鈍化誤聽的安全變體（本身不成詞；「動畫」「對話」刻意不收——同一錯字在他集映射到「多空」）
    ("入頓", "入鈍", [], []),
    ("次遁", "次鈍", [], []),
    ("動畫了", "鈍化了", [], []),
    # 三竹資訊（台股券商看盤系統資訊商）；三路 ASR 皆誤
    ("三足", "三竹", [], ["三足鼎立"]),
    ("回州", "回週", [], []),
    ("是周", "是週", [], []),
    ("收買", "收盤", [], ["收買人心"]),
    ("連長", "連漲", [], []),
    ("比較看細", "比較看戲", [], []),   # 不可用裸「看細」：看細節/看細微/看細的 全會誤傷
    ("被尬", "被軋", [], []),
    ("停水", "停損", [], []),
    ("點引", "點影", [], []),
    ("拆除", "拆出", [], []),
    ...
    ("教學指", "加權指", [], []),
    ("右手減", "有所減", [], []),
    ("它b回", "它逼回", [], []),   # 不可用裸「它b」：會把「它beta」切成「它逼eta」
]

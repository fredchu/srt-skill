...
- `cloud_asr.sh` 的流量預估搬到旗標解析之後並改成明確覆寫（`CLOUD_ASR_EST_HOURS／DOWN_GB／UP_GB`）。1.12.4 的寫法有兩個問題：
  `ASR_MODE` 在檔案開頭還沒解析，VV 永遠拿到 Breeze 的 6 GB；共用檔在開頭就 source、先把 `VAST_LIB_EST_*` 填成預設 5，
  後面的「沒值才設」永遠不生效。測試改成從假 vastai 記錄環境變數，實際驗到 `--vv` 送出 10 GB。
...
- **`vast_instance_lib.sh` 挑報價預設加 `static_ip=true`（`VAST_LIB_REQUIRE_STATIC_IP=0` 關掉）。**
  ...
  驗證：加上這個篩選後，4090 第一台（德州固定 IP，0.334 美元／小時）就用 bookcast 試跑書全程通過，57 段到齊、砍機確認。

## 1.12.1 - 2026-09-03

### 修正

- **`vast_instance_lib.sh` 挑報價的 CUDA 下限改 12.9（`VAST_LIB_MIN_CUDA`），對齊映像。** 12.8 的篩選放進了一台
  驅動 570（CUDA 12.8）的土耳其 4090，torch 報 Error 804「forward compatibility was attempted on non supported HW」
  ——GeForce 不能向前相容，驅動比映像（cuda-12.9）舊就是不能用；成功過的韓國 5090 是 13.0。當下 4090 有 23 台
  符合其他條件，只有 1 台卡在 12.8，收緊幾乎不損失。另一台香港 4090 在 loading 狀態 5 分鐘沒有任何拉映像訊息，
  被停滯偵測砍掉——那個機制第一次在真機上派上用場。
- **4090 在 Vast 上的實測結論（bookcast 試跑書，四台四敗）**：香港 loading 零訊息（停滯砍）、土耳其 CUDA 12.8（上面那條）、
  加州拉了 6 分鐘後卡住（停滯砍，該機網路 1.1 Gbps／硬碟 860 MB/s，是真卡）、馬來西亞容器 running 且直連埠存在但
  SSH 連 4 分鐘不通。同一時段韓國 5090 主機 5 次 5 過。**5090 留預設，4090 只當備援，別為了省每小時 0.1 美元去賭。**
  四次失敗都由 trap 砍機並回查確認，合計約 0.11 美元。
...
- **`subprocess.run(timeout=)` 超時送的是 SIGKILL，子程序的 trap 不會跑。**
  這裡呼叫的是 `subtitle.sh` → `cloud_asr.sh`，後者的 `trap cleanup` 就是砍機器那段——
  **所以超時會遺棄一台一直計費的機器，而且沒有任何訊息**。
  原本上限 600 秒也太短（今天實測慢機器光裝套件就 450 秒）。
  改成 Popen + terminate（給 trap 機會）+ 60 秒寬限 + kill，上限 2400 秒可調。

- **雲端模式下第一次 ASR 失敗要停整支。**
  原本失敗只 break 內層 buffer 迴圈，**外層會走到下一個異常段再開一台機器**。
  測試素材實際有 4 個異常段——「第一台失敗就停」根本不成立。
  本地模式不停：本地失敗不花錢。

- **`cloud_asr.sh` 的成本上限與重試上限都是「每次呼叫」的，不是整體的。**
  這支腳本對「每個異常段 × 每段三種 buffer」各呼叫一次，每次重新拿一份完整預算。
  加 `SRT_ASR_MAX_CALLS` 總量熔斷。

- **成功時的 stdout/stderr 被丟棄**，於是「這次走了哪條引擎」無從查證。
  改成落檔 `<output_dir>/_asr_call.log`。

### 新增

- **`--max-anomalies N`** — 雲端模式下每個異常段都是一台新機器，
  「先驗一段」是控制花費的正當做法。

### Live 驗證（US$0.037、180 秒、1 台機器）

素材：`2Q2026-美股展望` 第 5008.7 秒，「往」重複 **210 次**接 **26 秒斷層**，
同時命中兩個偵測判準，而且**真的吞掉了內容**（成品那條只寫「刪除重複幻覺」）。

| 判準 | 結果 |
|---|---|
| 真的走雲端 | ✅ log 有「引擎：runpod」、**本地模型呼叫 0 次** |
| 正規化在開機器之前 | ✅ 19:56:54 → 19:56:55 |
| 區間內條數 | 10（判準 ≥2） |
| 同字重複 | **210 → 5**（判準 <10） |
| 26 秒斷層 | ✅ **消失** |
| 機器 | ✅ 1 台、terminate success、GET 404、清單為空 |
| 原檔未被動到 | ✅ md5 不變 |

**救回的是真內容**：「Ficontech MISI 其實最大最大的 powerball 其實是 Ficontech」
這類技術名詞，不是填空。

### Known issues

- ⚠️ **Whisper large-v3 備援仍未上雲**（四條路的最後一條）。
  阻礙不是程式碼：雲端 large-v3 的逐字時間戳與本地相同那個證據，
  **是兩邊都不帶 initial_prompt 量的**，而生產配方會帶 prompt、雲端目前不支援。
  要做這條得先讓雲端支援 prompt，再重新量一次等價性。

## 1.9.1 - 2026-08-30

### 修復

- **能力探針的快取沒有欄位版本，升級後會沿用舊格式。**

  v1.9.0 加了 `breeze_reason` / `vv_reason` 兩個欄位。從舊版升級上來的機器
  **快取裡還是舊格式**，`subtitle.sh` 取 `breeze_reason` 取不到，就落到預設字串
  「原因：記憶體不足」——**而真因是「mlx_whisper 未安裝」。照那個訊息使用者會去買記憶體。**

  只比對 `host=` 不夠：主機沒變，變的是欄位。加了 `cache_schema`，不符就重算。

  ⚠️ **這個 bug 是發版清單第 7 步（在目標機器上跑發布的那個標籤）抓到的。**
  本機測不出來——本機的快取一直是新的。這是那份清單第一次真的抓到東西。
...
### 熱詞路徑已 live 驗證（4月-04、49 分鐘、US$0.14）

| 判準 | 結果 |
|---|---|
| **靶詞命中** | ✅ 帶 terms **33 次**，兩次不帶 terms 的基線各 **11 次**（門檻 ≥15） |
| **金絲雀** | ✅ 影片裡不存在的 `ZQXV Capital` **0 次**，模型沒有把提示詞抄進輸出 |
| 局部性 | ❌ **判準不成立**，見下 |

逐詞：HTO 1→8、VGNT 0→3、MarketCap / NOVTU / Papa John 各 0→2、
New Holding 4→7、Ron Baron 6→9。**9 個靶詞救回 7 個**（Ackman 與 Broadcom 仍 0）。

**局部性判準不成立，而且那件事本身是發現**：原本預期「效果局部、雜訊全域」，
實際上**熱詞會改變整支影片的措辭與斷句**——沒有靶詞的 5-10 分鐘桶也差 11.7%
（自噪 1.2%），段數 170 → 199。判準沒用，但它要回答的問題被靶詞計數加金絲雀答了。

⚠️ **`HTO` 出現 8 次但成品只有 4 次——熱詞可能誘發模型多插入那個詞。**
方向對不對需要人耳裁決，本輪未做。

### Known issues

- ⚠️ **等價性數字全部是「兩側都不帶 terms」的口徑。**
  跨側 10.6%、雲端自噪 1.8% 都是無 terms 下量的。
  **生產情境（兩側都帶 terms）的跨側差異未量**——而熱詞會改變全片 9% 的措辭，
  所以那個數字不能直接套用。本地帶 terms 那跑在 M1 Max 上不花錢，可補。
- ⚠️ **熱詞對 VibeVoice 是雙刃**：救回專有名詞，但改變全片措辭、且可能誘發插入。
  跟 `SKILL.md` 的 slide terms 警告（raw OCR 術語反而拉低英文專名辨識 F1 0.45→0.41）
  是同一件事的另一面。

...
- **ASR 引擎**：**預設 `mlx`（本地 Apple Silicon）**。用戶提到「用 runpod」「跑雲端」「M1 Max 不在」→ `--engine=runpod`；「用 vast」「上 vast.ai」→ `--engine=vast`（Vast.ai 段見下）
  ...
  **Vast.ai**（2026-09-03 起）：用戶說「用 vast」「上 vast.ai」→ 加 `CLOUD_ASR_PROVIDER=vast`。先搜報價再開
  （預設 RTX 5090、每小時上限 0.6 美元、依「GPU 時數＋流量」預估總費用排序、只挑固定 IP 主機 `static_ip=true`、排除 CN/VN 與 137.175./207.246.98./144.202.115. 這些「IP 在美國、
...
2. **平行發起所有 subagent**：用單一訊息同時發起所有 Agent tool 呼叫：
   ...
   - 完成後回報修改了多少條；回報中提及任何具體條目（疑點、代表性修正）時**必須附該條時間軸（HH:MM:SS），不得只給條目編號**（編號經 merge 會重編，主 agent 無法還原）
   ```
...
- **未收斂項一律按時間順序由前到後列出**（2026-08-15 用戶要求）：不要按 sidecar 檔案順序、不要按段號、不要按證據層級分組。用戶是拿這份清單逐條回聽原音裁決的，時間亂跳會讓他在時間軸上來回找。每列一行 `HH:MM:SS ｜ 字幕現況 ｜ 候選與理由`，極性相反（一路有「不」另一路沒有）的項目要標出來，那類最容易看走眼。
...
`01:15:56` 之後的條目全報成 `00:16:01`，差 3600 秒）、**段內相對時間混進正常時間軸**
（8月-05 段 3 寫成「00:29:47（09:47附近）」）、**憑印象報錯位置**（8月-05 複查把只出現在
`00:25:02` 的句子報成 `00:09:34`）。三次都靠下面這條規則接住，成品沒有受影響。
檔案本身的時間軸每次都是對的——錯的只有 subagent 自由文字裡的描述，所以不必重跑校正，
只要不採信回報文字即可。

**時間點 hard rule（2026-07-24 技術分析-7月-03 用戶抓到兩處報錯時間後立規）**：報告中出現的
**每一個時間點**（疑點、修正、抽查建議）都必須「拿引用文字 grep final SRT」解出實際時間軸後填入。
**禁止**由條目編號、段落位置或任何比例心算換算時間——subagent 自由文字常回報 cue 編號（如
「415-416 有語意疑點」），cue 編號經 merge/postprocess 已重編且與時間非線性對應，腦補換算
必錯（實例：cue 415 估成 ~26 分，實際 17:26；cue 216 估成 16:12，實際 8:47）。grep 找不到
引用文字時，照原文回報並標註「時間未定位」，不要給估算值。
